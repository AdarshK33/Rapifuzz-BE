...
const msToTimeReEdited = (msDuration) =>{
  ...
  let mm = parseInt(minutes)>1 ? minutes+" min ": parseInt(minutes) > 0 ? minutes+ " min ": ""
  let ss = parseInt(seconds)>1 ? seconds+" sec": parseInt(seconds) > 0 ? seconds+ " sec": ""
  return hh+''+mm;

}

exports.getDateTimeDifference = (delivery_date, slot_from, dispatch_date_time) => {
...
exports.getDateTimeDifference_without_slotfrom = (delivery_date_time, dispatch_date_time) => {
...
exports.compareWithCurrentTime = (delivery_date, slot_from, dispatch_date_time) => {
  const delivery_date_time = new Date(delivery_date + ' ' + slot_from);
  dispatch_date_time = new Date(dispatch_date_time);
  ...
  if(dispatch_date_time > cur_date_time){
    const eta = this.getDateTimeDifference_without_slotfrom(delivery_date_time, dispatch_date_time)
    return { eta, live_eta_update: false  }
  }else if(dispatch_date_time <= cur_date_time && cur_date_time < delivery_date_time){
    const eta = this.getDateTimeDifference_without_slotfrom(delivery_date_time, cur_date_time)
