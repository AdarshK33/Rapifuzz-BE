...
UserToken.isValidToken = (user_id, token_id) => {
  return new Promise((resolve, reject) => {
    sql.query(`SELECT * FROM user_tokens WHERE user_id = ? AND token_id = ? AND valid_till > NOW()`,[user_id, token_id], (err, res) => {
      if (err) {
        return reject(false);
      }
      if(res.length == 0){
...
UserToken.deleteToken = (user_id, token_id) => {
  return new Promise((resolve, reject)=> {
    sql.query(`DELETE FROM user_tokens WHERE user_id = ? AND token_id = ?`, [user_id, token_id], (err, res)=> {
      if(err)reject(null)
      resolve(res)
    })
  })
